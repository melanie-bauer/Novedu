  ...
  const renderTutorCard = (tutor: TutorConfig, zone: Visibility) => {
    ...
    return (
      <div
        ...
        className={cn(
          'relative w-52 rounded-lg bg-background px-3 py-2.5 shadow-sm cursor-grab active:cursor-grabbing transition-colors',
          zone === 'private' && isDraft ? 'border border-dashed border-muted-foreground/30 bg-muted/20' : 'border border-border',
          'group'
        )}
      >
        <div className="absolute right-2 top-2 flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
          <button
            onClick={(e) => { e.stopPropagation(); navigate(`/app/tutors/${tutor.id}/edit`); }}
          ...
          <button
            onClick={(e) => { e.stopPropagation(); /* TODO: delete action */ }}
            className="p-1 rounded hover:bg-muted"
            aria-label="Löschen"
          >
        ...
        <div className={cn('h-full flex flex-col justify-between', 'group-hover:border-black group-hover:border-solid')}>
          <div>
    ...
    <div className="h-full min-h-0 flex flex-col gap-4 p-4 lg:p-6 bg-muted/20 overflow-y-auto scrollbar-thin">
      <Card>
        <CardHeader className="flex items-start justify-between">
          <div>
            <CardTitle className="text-lg">Tutor verwalten</CardTitle>
            <p className="text-sm text-muted-foreground">Griff links greifen und in eine Spalte oder auf eine Klasse ziehen.</p>
          </div>
        </CardHeader>
        <CardContent className="grid grid-cols-1 gap-4 md:grid-cols-[1fr] lg:grid-cols-[minmax(0,1.4fr)_minmax(280px,0.6fr)]">
          <div className="space-y-4">
            {visibilitySections.map((zone) => {
              const ZoneIcon = zone.icon;

              return (
                <div key={zone.key} className="rounded-md border p-0 bg-background">
                  <div className="flex items-start justify-between gap-3 px-4 py-3 border-b bg-muted/20">
                    <div>
                      <div className="flex items-center gap-2">
                        ...
                        <Badge variant="secondary" className="ml-2">{tutorsByVisibility[zone.key].length}</Badge>
                      </div>
                      <p className="text-xs text-muted-foreground mt-1">{zone.description}</p>
                    </div>
                    {zone.key === 'private' ? (
                      <Button size={"sm"} onClick={() => navigate('/app/tutors/new')} className="gap-2">
                        <Plus className="w-4 h-4" />
                        Neuer Tutor
                      </Button>
                    ) : null}
                  </div>
                  <div className="p-4 min-h-[4rem] max-h-[22rem] overflow-auto" onDragOver={(e) => e.preventDefault()} onDrop={() => onDropToVisibility(zone.key)}>
                    {tutorsByVisibility[zone.key].length === 0 ? (
                      <div className="rounded-xl border border-dashed bg-muted/20 px-4 py-6 text-sm text-muted-foreground text-center">Tutor hierhin ziehen...</div>
                    ) : (
          ...
          <Card className="min-h-0 flex flex-col overflow-hidden xl:max-h-[calc(100vh-11rem)]">
            <CardHeader className="pb-3 border-b bg-muted/20">
              ...
              <div className="relative">
                ...
                <Input value={classSearch} onChange={(e) => setClassSearch(e.target.value)} className="pl-9" placeholder="Klassen suchen (z.B. 4, ahif, ...)" />
              </div>
            ...
            <CardContent className="min-h-0 overflow-auto space-y-3 pt-4">
              {filteredClasses.map((classItem) => {
                ...
                return (
                  <div key={classItem.id} onDragOver={(e) => e.preventDefault()} onDrop={() => onDropToClass(classItem.id)} className="rounded-xl border p-3 bg-card shadow-sm">
                    ...
                    {assignedTutors.length === 0 ? (
                      ...
                      <div className="space-y-1.5">
                        {assignedTutors.map((tutor) => (
                          <div key={`${classItem.id}-${tutor.id}`} className="rounded-md border bg-muted/30 px-2 py-1.5 text-xs flex items-center justify-between gap-2">
                            <div className="min-w-0">
                              <p className="truncate text-sm font-medium">{tutor.icon} {tutor.name}</p>
                              <p className="text-xs text-muted-foreground truncate">{tutor.subject} • {getTeacherName(tutor.createdBy)}</p>
                            </div>
                            <Button variant="ghost" size="icon-sm" onClick={() => confirmRemoveTutorFromClass(tutor.id, classItem.id)} aria-label="Tutor von Klasse entfernen"><X className="w-3 h-3" /></Button>
                          </div>
                        ))}
                      </div>
                    )}
              ...
              {filteredClasses.length === 0 && (<p className="text-sm text-muted-foreground">Keine Klassen gefunden.</p>)}
            </CardContent>
          </Card>
        </CardContent>
      </Card>
      ...
      <AlertDialog open={!!pendingRemoval} onOpenChange={() => setPendingRemoval(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>Möchtest du "{pendingRemoval?.tutorName}" wirklich aus "{pendingRemoval?.className}" entfernen?</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
